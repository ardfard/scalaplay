...
class ListsSuite {
  ...
  @Test def `sum with negative numbers`: Unit = {
    assertEquals(sum(List(-1, -3, 5)), 1)
  }

  @Test def `sum with zeros`: Unit = {
    assertEquals(sum(List(0, 0, 5)), 5)
  }

  @Test def `sum with zero elements`: Unit = {
    assertEquals(0, sum(List.empty[Int]))
  }

  @Test def `max with repeated elements`: Unit = {
    assertEquals(max(List(1, 2, 2, 2, 5, 5)), 5)
  }

  @Test def `max with empty list raise execption`: Unit = {
    try {
      max(List.empty)
