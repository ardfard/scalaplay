...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Arr(items) => {
        def seq(xs: List[Json]): Option[List[A]] = {
          xs match {
            case Nil => Some(Nil)
            case head :: next => for {
              value <- decoder.decode(head)
              res <- seq(next)
            } yield (value::res)
          }
        }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Obj(fields) => fields.get(name).flatMap(decoder.decode)
      case _           => None
    }
...
trait ContactsCodecs {
  ...
  implicit val contacsDecoder: Decoder[Contacts] =
    Decoder.fromFunction(json =>
      Decoder
        .field[List[Person]]("people")
        .decode(json)
        .flatMap(persons => Some(Contacts(persons)))
    )
}
