...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    ...
    val mostLeaf =
      if (rightMost.retweets > leftMost.retweets) rightMost else leftMost
    if (elem.retweets > mostLeaf.retweets) elem else mostLeaf
  }
...
object GoogleVsApple {
  ...
  private def allTweetsContains(list: List[String]) =
    allTweets.filter(tweet => list.exists(tweet.text.contains(_)))

  lazy val googleTweets: TweetSet = allTweetsContains(google)

  lazy val appleTweets: TweetSet = allTweetsContains(apple)
