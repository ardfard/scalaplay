...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    val mapped = chars.foldLeft(Map.empty[Char, Int])((m, c) => {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    def insert(tree: CodeTree, trees: List[CodeTree]): List[CodeTree] =
      trees match {
        case Nil => List(tree)
        case x :: xs =>
          if (weight(x) >= weight(tree)) tree :: x :: xs
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    val trees = (times _ andThen makeOrderedLeafList)(chars)
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def check(t: CodeTree, acc: List[Char]) =
      t match {
        case Leaf(char, _) => (tree, acc :+ char)
        case st            => (st, acc)
      }
    bits
      .foldLeft((tree, List.empty[Char]))((st, b) => {
        val (t, res) = st
        t match {
          case Fork(l, r, _, _) =>
            if (b == 0) check(l, res) else check(r, res)
          case Leaf(char, _) => (tree, res :+ char)
        }
      })
      ._2
  }
  // {
  //   def go(ct: CodeTree, cb: List[Bit], res: List[Char]): List[Char] =
  //     ct match {
  //       case Leaf(char, _) => go(tree, cb, res :+ char)
  //       case Fork(l, r, _, _) =>
  //         cb match {
  //           case Nil => res
  //           case x :: xs => {
  //             println(x)
  //             if (x == 0) go(l, xs, res) else go(r, xs, res)
  //           }
  //         }
  //     }
  //   go(tree, bits, List.empty)
  // }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def contains(tree: CodeTree, c: Char) =
      tree match {
        case Leaf(char, weight)   => char == c
        case Fork(_, _, chars, _) => chars.contains(c)
      }
    def encode_(tree: CodeTree, c: Char, bits: List[Bit]): List[Bit] =
      tree match {
        case Fork(left, right, _, _) =>
          if (contains(left, c)) encode_(left, c, bits :+ 0)
          else encode_(right, c, bits :+ 1)
        case Leaf(char, weight) => bits
      }
    text.flatMap(c => encode_(tree, c, List.empty))
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def go(tree: CodeTree, bits: List[Bit]): CodeTable =
      tree match {
        case Leaf(char, _) => List((char, bits))
        case Fork(left, right, _, _) =>
          mergeCodeTables(go(left, bits :+ 0), go(right, bits :+ 1))
      }
    go(tree, List.empty)
  }
